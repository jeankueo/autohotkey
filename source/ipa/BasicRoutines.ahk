...
if (sHotKeyOnL <> "" && sHotKeyOnL <> "(none)")
  Hotkey, %sHotKeyOnL%, DoHotkeyOn
if (sHotKeyOffL <> "" && sHotKeyOffL <> "(none)" )
  Hotkey, %sHotKeyOffL%, DoHotkeyOff
...
SendUnicode( p_uchar )     ; 3 DllCalls, needs one prior call to SendUinit()
{
   ...
   Critical   
   Transform Clipboard, Unicode, %AText%
   ClipWait 1
   SendInput ^v
   Sleep, 50
...
SendUnicodeString( p_text ) 
{
  ...
  loop %count%
  {
   ...
   Critical   
   Transform Clipboard, Unicode, %out%
   ClipWait 1
   SendInput ^v
   Sleep, 50
...
CreateMenuAndDialog()
{
  ...
  Menu, Menu1, Add 
  Menu, Menu1, Add, Compatibility mode, CompMode
...
CreateHotkeys()
{
  ...
  if (sHotKeyOnL <> "" && sHotKeyOnL <> "(none)")
    Hotkey, %sHotKeyOnL%, off
  if (sHotKeyOffL <> "" && sHotKeyOffL <> "(none)" )
    Hotkey, %sHotKeyOffL%, off
...
CheckIfAutostart()
{
  ...
  RegRead, re, REG_SZ,HKEY_CURRENT_USER,Software\Microsoft\Windows\CurrentVersion\Run,%A_ScriptName%
  if (re <> "")
    iAutostartOn := 1
  else
    iAutostartOn := 0  
...
GetOutputSequence(group, pos)
{
  ...
  ende   := InStr(group, search ,false,pos) 
  ;msgbox ende %ende%
  length := ende - pos	
...
ReplaceValue(group, key, newkey, insert)
{
  global
  StringCaseSense, On
...
SendKey(what, back)
{
  ...
  if (back = 1)       ; Backspace character needed?
    Send {BS}
	
  StringLeft, t, what, 1
  if (t == "$")       ; Is it a ansi string?
  {
    ...
    if (StrLen(what) == 6)  ; Single character
	    SendUnicode(what)
	else
	{
...
; if no replacement is found then newkey is put out.
ReplaceLastKey(Group, newkey) 
{
  Global LastOutput
  ...
  {  
    ;msgbox x %x%
    x := ReplaceValue(group, LastOutput, newkey, 1)
    return x
  }	
  else
  {
...
; Syntax : ReplaceDoubleKey(Group, key)
ReplaceDoubleKey(Group, newkey) 
{
  Global LastOutput, LastKey
  if (LastOutput <> "")
  {  
    ss = %LastOutput% %LastKey%
    x := ReplaceValue(group, ss, newkey, 1)
	; msgbox %x%
    return x
  }	
  else
...
ReplaceKeyOpen(group, key, newkey)
{
  ;msgbox %key%
  x := ReplaceValue(group, key, newkey, 1)
  return x
}  

ExchangeKey(Group, newkey) 
{
  Global LastKey
  x := ReplaceValue(group, LastKey, newkey, 0)
  return x
}  

ExchangeDoubleKey(Group, newkey) 
{
  Global LastOutput, LastKey
  if (LastOutput <> "")
  {  
    ss = %LastOutput% %LastKey%	
    x := ReplaceValue(group, ss, newkey, 0)
    return x
  }	
  else
  {
...
; Replaces the last character 
ExchangeKeyOpen(group, key, newkey)
{
  ;msgbox %key%
  x := ReplaceValue(group, key, newkey, 0)
  return x
}  
...
MatrixOut(Group, key, default, offset, new)
{
 ...
 { 
    ...
    if (MatPos <> -1) ; Key found?
    {
      ...
      check := SubStr(Group, x, 7)
	;msgbox x %x%
	;msgbox check %check%
	  t := GetOutputSequence(group,x + 2)                       ; Get element
	;msgbox t %t%
      SendKey(t, new)                                           ; Put it out
	  return true
    }
    else
	{
...
MatrixBack(Group, key, default, offset, new)
{
  Global
  StringCaseSense, On
  if (key <> "")    ; New key provided?
